...
const Layout: React.FC<LayoutProps> = ({ children }) => {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-gray-50 via-amber-50/30 to-yellow-50/20 transition-all duration-300">
      {/* Header */}
            ...
            <div className="flex items-center space-x-2 sm:space-x-4">
              ...
              <div className="relative">
                ...
                >
                  ...
                  <span className="hidden sm:block font-medium">{user?.fullName || t('common.admin')}</span>
                ...
                {headerMenuOpen && (
                  <div
                    id="header-menu"
                    className="absolute right-0 mt-2 w-48 bg-white/95 rounded-xl shadow-soft-lg border border-amber-200/20 py-2 z-50 animate-scale-in backdrop-blur-md"
                    role="menu"
                    aria-orientation="vertical"
                  >
                    <div className="px-4 py-2 border-b border-amber-200/30">
                      <p className="text-sm font-medium text-gray-900">{user?.fullName || t('common.adminUser')}</p>
                      <p className="text-xs text-amber-600">{user?.email || 'user@example.com'}</p>
                      <p className="text-xs text-gray-500 capitalize mt-1">{user?.role?.replace('_', ' ') || ''}</p>
                    </div>
                    {/* <Link
                      to="/profile"
                      className="flex items-center px-4 py-2 text-sm text-gray-700 hover:bg-gradient-to-r hover:from-amber-50 hover:to-yellow-50 hover:text-amber-700 focus:outline-none focus:bg-amber-50 transition-all duration-200"
                      role="menuitem"
                    >
                    ...
                    <div className="border-t border-amber-200/30 mt-2 pt-2">
                      <button 
                        onClick={async () => {
                          await logout();
                          window.location.href = '/login';
                        }}
                        className="flex items-center w-full px-4 py-2 text-sm text-red-600 hover:bg-red-50 focus:outline-none focus:bg-red-50 transition-all duration-200"
                        role="menuitem"
                      >
                        {t('common.signOut')}
                      </button>
      ...
      <div className="flex pt-16">
        ...
        <nav
          ...
          className={`
            ...
            ${sidebarOpen ? 'w-64' : 'w-16'} bg-white/95 shadow-soft-lg curved-sidebar sidebar-transition backdrop-blur-md border-r border-amber-200/20
          ...
          <div className="flex flex-col h-full">
                ...
                <div className="flex items-center justify-between">
                  ...
                  {!isMobile && (
                    <button
                      ...
                      className="toggle-button p-1 text-amber-600 hover:text-amber-700 rounded-lg focus:outline-none focus:ring-2 focus:ring-amber-500 focus:ring-opacity-50 hover:bg-amber-50"
            ...
            {!sidebarOpen && !isMobile && (
              <div className="p-4 border-b border-amber-200/30">
                <button
                  onClick={toggleSidebar}
                  className="toggle-button p-2 text-amber-600 hover:text-amber-700 rounded-lg focus:outline-none focus:ring-2 focus:ring-amber-500 focus:ring-opacity-50 w-full flex justify-center hover:bg-amber-50 hover:shadow-glow"
                  ...
                  return (
                    <Link
                      ...
                      className={`
                        ...
                        ${isActive(item.href)
                          ? 'bg-gradient-to-r from-amber-100 to-yellow-100 text-amber-700 border-r-4 border-amber-500 shadow-soft shadow-glow'
                          : 'text-gray-600 hover:bg-gradient-to-r hover:from-amber-50 hover:to-yellow-50 hover:text-amber-700 hover:shadow-soft'
                        }
                      `}
